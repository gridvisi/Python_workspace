...
前景提要： 之前介绍过一个绘制手绘风格图形的工具cutecharts：一款蠢萌蠢萌的可视化工具
...
with plt.xkcd(
    ...
    patches, texts, autotexts = plt.pie(
        x = [1, 2, 3, 4, 5],  #返回三个对象
        labels=[ 'c++', 'python','vex robot','arduino',"scratch"],
    ...
    plt.legend(
        patches,
        ['A', 'B', 'C'],  #添加图例
        title= "dingdingmao code_camp", #"丁丁猫亲子创客",
        loc="center left",
        fontsize=15,
        bbox_to_anchor=(1, 0, 0.5, 1))

    plt.title('dingdingmao course Enrollment pie', size=20)
    plt.show()
