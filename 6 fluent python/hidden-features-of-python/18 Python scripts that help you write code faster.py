...
The following snippet can be used to convert a string to title case. 
This is done using the title() method of the string class.
'''
...
You can use multiplication (*) with strings or lists. 
...
# [1,2,3,1,2,3,1,2,3]
'''
An interesting use case of this could be to define 
a list with a constant value — let’s say zero.
'''
n = 4
...
Python makes it quite simple to swap values between two variables without using 
another variable.
'''
a = 1
b = 2
...
We have already discussed how to reverse a string. 
So palindromes become a straightforward program in Python.
'''
my_string = "abcba"
...
The following script uses enumerate to iterate through values in a 
list along with their indices.
'''
my_list = ['a', 'b', 'c', 'd', 'e']
...
The following snippet generates nnumber of random samples from 
a given list using the random library.
'''
import random
...
The following snippet uses the timelibrary to calculate the time taken to 
